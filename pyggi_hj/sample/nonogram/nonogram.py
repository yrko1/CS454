...
def make_dnf(comb, row_flag, l):
    if row_flag:
        ...
        tmp_list = range(1+l,1+row_num*col_num-1,row_num)
    tmp = form()
    if len(comb) == 1:
...
def dnf2cnf(ands_list):
    if type(ands_list[0]) != type([]):
        a = []
        a.append(ands_list)
        ands_list = a
    new_list = []
    global variable_num
    new_list.append([str(e) for e in range(variable_num,variable_num+len(ands_list))])
    for ands in ands_list:
        for a in ands:
            tmp = ['-'+str(variable_num),a]
            new_list.append(tmp)
        variable_num += 1
...
subprocess.call('rm dummy', shell=True)
