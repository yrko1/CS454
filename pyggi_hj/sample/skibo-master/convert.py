...
def generate(string, heuristic, showInfo, showComments):
	#print('convert.py generate',string)
	# STEP 0 :
	# -----
	# Sanitize
	...
	lines = list(filter(lambda l : l not in [' ',''] , clean.strip().split('\n')))
	...
	if showInfo :
		print (" CNF Infos : \n-----------")
		print (" %r variables and %r clauses " % (ilen(variables), ilen(clauses)))
		print ()
